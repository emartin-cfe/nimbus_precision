...
syntax_error = "Correct syntax: ./column_precision.R <CSV> <'Average' or 'CV' or 'Paired_Ratio'> <output_png>"
...
if(data_type == "CV") { main_title = "Median RFU CV by column over time"; y_label = "Median RFU CV"; log_scale=""; y_lim=c(0,1); colls = seq(1,12,2) }
if(data_type == "Paired_Ratio") { main_title = "Dynamic range for avg RFU across columns"; y_label = "Neg Avg RFU / Pos Avg RFU"; log_scale=""; y_lim=c(0,0.1); colls = seq(1,12,2); }

png(fname,width=800,height=500)
par(mar=c(6, 5, 4, 1))
plot(NA, xlim=c(1,length(dates)), ylim=y_lim, axes=FALSE, ann=FALSE, xaxs="r", log=log_scale)
if(data_type == "Average") { mtext(text="Blue = +ive flouroscein (1/3/5/7/9/11)     Red = -ive PBS (2/4/6/8/10)",side=3) }
